...
function renderFinalQuestion(question, result = {}) {
  ...
  const relationFeedback = result.relationAttempts
    ? relationCorrect
      ...
      : `<div class="spec-feedback is-error" role="status" aria-live="polite"><strong>もう一度、する / されるを考えよう。</strong>${result.relationAttempts >= 2 ? '<p class="spec-hint">Hint: 名詞と元の動詞の向きを見てみよう。</p>' : ''}${result.relationAttempts >= 3 ? `<p>正解の関係：${escapeHtml(question.relationAnswer === 'active' ? `${question.noun} → ${question.verb}` : `${question.verb} → ${question.noun}`)}</p>` : ''}</div>`
    : '';
...
function renderDragRuleQuestion(question, result = {}) {
  ...
  const feedback = result.correct
    ...
    : result.error
      ? `<div class="spec-feedback is-error" role="status" aria-live="polite"><strong>もう一度、関係を確認しよう。</strong>${result.attempts >= 2 ? '<p class="spec-hint">Hint: する側は -ing、される側は p.p.</p>' : ''}</div>`
      : '';
...
function renderFinalQuiz(stage, state) {
  ...
  const wrongQuestions = questions.filter((question) => state.questions[question.id]?.formCorrect !== true);
...
function renderActivePassiveStage(state) {
  ...
  return `<div class="spec-active-passive-stage"><article class="spec-example-panel"><p class="spec-kicker">ACTIVE</p><p class="spec-example-sentence">a boy <strong>kicking</strong> the ball</p>${renderRelationArrow({ from: 'boy', to: 'ball', label: 'ACTIVE / する', direction: 'forward' })}<button class="spec-node-button" type="button" data-spec-action="active-reveal">boy</button>${active ? '<div class="spec-feedback is-success" role="status"><strong>Who kicks?</strong><p>the boy</p><p>boy が kick する<br /><b>ACTIVE → kicking</b></p></div>' : ''}</article><article class="spec-example-panel"><p class="spec-kicker">PASSIVE</p><p class="spec-example-sentence">the ball <strong>kicked</strong> by the boy</p>${renderRelationArrow({ from: 'boy', to: 'ball', label: 'PASSIVE / される', direction: 'forward' })}<p class="spec-question-prompt">ball は kick する？<br />kick される？</p><div class="spec-choice-grid"><button class="spec-choice" type="button" data-spec-action="passive-relation" data-spec-value="active" aria-pressed="${state.passiveRelation === 'active'}">する</button><button class="spec-choice" type="button" data-spec-action="passive-relation" data-spec-value="passive" aria-pressed="${passive}">される</button></div>${state.passiveRelation === 'active' ? '<p class="spec-feedback is-error" role="status"><strong>Not yet.</strong><br />ball は kick される側です。</p>' : ''}${passive ? '<div class="spec-feedback is-success" role="status"><strong>ball は kick される</strong><p>PASSIVE → <b>kicked</b></p></div>' : ''}</article></div>${active && passive ? '<button class="spec-action-button" type="button" data-spec-action="complete-stage">次へ →</button>' : ''}</div>`;
}
...
function renderOneWordStage() {
  return `<div class="spec-one-word-stage"><div class="spec-large-sentence">the <strong class="spec-highlight">laughing</strong> children</div><p class="spec-note">laughing だけなら1語。</p><p class="spec-rule-emphasis">原則：1語 → 名詞の前</p><button class="spec-action-button" type="button" data-spec-action="complete-stage">位置を覚えた →</button></div>`;
}

function renderBuildPhraseStage(state) {
  return `<div class="spec-build-stage"><p class="spec-note">laughing にこの情報を追加してみよう。</p>${renderSentenceBuilder({ noun: 'children', modifier: 'laughing', extra: 'at the clown', built: state.phraseBuilt })}${state.phraseBuilt ? '<div class="spec-feedback is-success" role="status"><strong>説明が長くなったので、名詞の後ろへ移動しました。</strong><p class="spec-rule-emphasis">原則：2語以上 → 名詞の後ろ</p><p class="spec-large-sentence">the children <strong class="spec-highlight">laughing at the clown</strong></p></div>' : ''}<button class="spec-action-button" type="button" data-spec-action="complete-stage" ${state.phraseBuilt ? '' : 'disabled'}>次へ →</button></div>`;
}

function renderPositionCompareStage(stage, state) {
  return `<div class="spec-position-stage"><div class="spec-toggle-row"><button type="button" class="spec-toggle${state.positionMode !== 'long' ? ' is-selected' : ''}" data-spec-action="position-toggle" data-spec-value="short">1 WORD</button><button type="button" class="spec-toggle${state.positionMode === 'long' ? ' is-selected' : ''}" data-spec-action="position-toggle" data-spec-value="long">2+ WORDS</button></div><div class="spec-position-example"><p class="spec-large-sentence">${state.positionMode === 'long' ? 'the children ' : 'the '}<strong class="spec-highlight">${state.positionMode === 'long' ? 'laughing at the clown' : 'laughing'}</strong>${state.positionMode === 'long' ? '' : ' children'}</p><p>${state.positionMode === 'long' ? '説明が長いので名詞の後ろ。' : '1語なので名詞の前。'}</p></div><div class="spec-rule-cards"><div><code>the pen</code> + <code>on the desk</code><strong>→ the pen on the desk</strong></div><div><code>time</code> + <code>to study</code><strong>→ time to study</strong></div><div><code>the boy</code> + <code>who is tall</code><strong>→ the boy who is tall</strong></div></div><p class="spec-explanation-line">これは分詞だけのルールではありません。<br /><b>英語では、長い説明を名詞の後ろに置くことが多い。</b></p>${renderAdvancedDrawer(stage.advanced, { id: 'l3-one-word-after' })}<button class="spec-action-button" type="button" data-spec-action="complete-stage">比較できた →</button></div>`;
}
...
function renderCase(caseData, index, state) {
  ...
  const relationChoices = `<button type="button" class="spec-choice" data-spec-case-relation="${index}:active">${caseData.relationAnswer === 'active' ? 'YES' : 'する'}</button><button type="button" class="spec-choice" data-spec-case-relation="${index}:passive">${caseData.relationAnswer === 'passive' ? 'YES' : 'される'}</button>`;
  return `<article class="spec-case-panel"><p class="spec-kicker">CASE ${index + 1}</p><p class="spec-example-sentence">${escapeHtml(caseData.sentence)}</p><div class="spec-case-nodes"><button type="button" class="spec-node-button${current.nounClicked ? ' is-selected' : ''}" data-spec-case-node="${index}:noun">${escapeHtml(caseData.noun)}</button><span>＋</span><button type="button" class="spec-node-button${current.verbClicked ? ' is-selected' : ''}" data-spec-case-node="${index}:verb">${escapeHtml(caseData.verb)}</button></div>${current.nounClicked && current.verbClicked ? `<p class="spec-question-prompt">${escapeHtml(caseData.noun)} が ${escapeHtml(caseData.verb)} する？</p><div class="spec-choice-grid">${relationChoices}</div>` : '<p class="spec-note">名詞と動詞をクリックして、関係を見ます。</p>'}${current.relation ? `${renderRelationArrow({ from: current.relation === 'active' ? caseData.noun : caseData.verb, to: current.relation === 'active' ? caseData.verb : caseData.noun, label: current.relation === 'active' ? 'ACTIVE / する' : 'PASSIVE / される' })}<p class="spec-bridge-sentence">${escapeHtml(caseData.relationSentence)}</p><div class="spec-choice-grid">${caseData.forms.map((form) => `<button type="button" class="spec-choice${current.form === form.id ? ' is-selected' : ''}" data-spec-case-form="${index}:${escapeHtml(form.id)}" aria-pressed="${current.form === form.id}">${escapeHtml(form.text)}</button>`).join('')}</div>` : ''}${current.error ? '<p class="spec-feedback is-error" role="status"><strong>もう一度、関係を見てみよう。</strong></p>' : ''}${complete ? `<p class="spec-feedback is-success" role="status"><strong>${escapeHtml(caseData.formAnswer)}</strong> — ${escapeHtml(caseData.relationReason)}</p>` : ''}</article>`;
}
...
function renderRapidJudgeStage(stage, state) {
  const data = stage.rapid ?? {};
  const selectedRelation = state.rapidRelation;
  const selectedForm = state.rapidForm;
  const done = selectedRelation === data.relationAnswer && selectedForm === data.formAnswer;
  return `<div class="spec-rapid-stage"><article class="spec-rapid-card"><p class="spec-example-sentence">${escapeHtml(data.sentence)}</p><p class="spec-kicker">STEP 1 · 関係判定</p>${renderSVRelationJudge({ noun: data.noun, verb: data.verb, relation: selectedRelation, form: selectedForm, forms: data.forms })}${state.rapidError ? '<p class="spec-feedback is-error" role="status"><strong>関係または形をもう一度確認しよう。</strong></p>' : ''}${done ? `<p class="spec-feedback is-success" role="status"><strong>${escapeHtml(data.noun)} → ${escapeHtml(data.verb)} → ${escapeHtml(data.formAnswer)}</strong><br />関係判定 → 形選択の順です。</p>` : ''}</article><details class="spec-advanced"><summary>BONUS：being p.p.</summary><div class="spec-advanced-body"><p>The chocolate cake <b>being baked</b> in the oven right now ...</p><p>受動＋進行 = <strong>being baked</strong></p></div></details><details class="spec-advanced"><summary>BONUS：waiting room</summary><div class="spec-advanced-body"><p>room が wait する？ → NO</p><p>SV関係が成立しないので、これは分詞ではありません。waiting room =「待つための部屋」</p></div></details><button class="spec-action-button" type="button" data-spec-action="complete-stage" ${done ? '' : 'disabled'}>次へ →</button></div>`;
}
...
function renderEmotionIntroStage(stage, state) {
  return `<div class="spec-emotion-intro-stage">${renderChoiceQuestion(stage.question, state.questions[stage.question.id] ?? {})}${state.questions[stage.question.id]?.correct ? `<div class="spec-emotion-callback"><p class="spec-large-sentence">surprise 人</p>${renderRelationArrow({ from: 'surprise', to: '人', label: 'CAUSE / 驚かせる' })}<strong>人を驚かせる</strong><p>He surprised his wife by giving her flowers.</p></div>` : ''}</div>`;
}

function renderEmotionSwitchStage(state) {
  const mode = state.emotionMode ?? 'boring';
  return `<div class="spec-emotion-switch-stage"><div class="spec-emotion-visual">${renderRelationArrow({ from: 'MOVIE', to: 'ME', label: 'boredom / 感情' })}</div><p class="spec-note">左は感情を与える側。右は感情を与えられる側。</p><div class="spec-toggle-row"><button type="button" class="spec-toggle${mode === 'boring' ? ' is-selected' : ''}" data-spec-action="emotion-toggle" data-spec-value="boring">boring</button><button type="button" class="spec-toggle${mode === 'bored' ? ' is-selected' : ''}" data-spec-action="emotion-toggle" data-spec-value="bored">bored</button></div><div class="spec-emotion-result"><p class="spec-large-sentence">You are <strong class="spec-highlight">${mode}</strong>.</p>${mode === 'boring' ? '<p>You → 周りの人を退屈させる</p><p>「あなたは退屈な人だ」</p>' : '<p>周りの状況 → Youを退屈させる</p><p>「あなたは退屈している」</p>'}</div><div class="spec-myth"><strong>主語が人なら -ed？</strong><div class="spec-choice-grid"><button type="button" class="spec-choice" data-spec-action="myth-choice" data-spec-value="yes">YES</button><button type="button" class="spec-choice" data-spec-action="myth-choice" data-spec-value="no">NO</button></div>${state.mythChoice === 'no' ? '<p class="spec-feedback is-success" role="status"><strong>NO。</strong><br />人でも、周りに感情を与える側なら -ing。<br /><b>Oliver is exciting. / You are amazing!</b></p>' : state.mythChoice ? '<p class="spec-feedback is-error" role="status"><strong>もう一度。</strong>主語が人かどうかだけでは決まりません。</p>' : ''}</div><button class="spec-action-button" type="button" data-spec-action="complete-stage" ${state.mythChoice === 'no' ? '' : 'disabled'}>次へ →</button></div>`;
}
...
function renderStage(stage, state) {
  switch (stage.kind) {
    ...
    case 'summary': return `<div class="spec-summary-stage">${renderLessonCompletePanel({ completed: state.lessonCompleted, summary: state.summary ?? '' })}${renderRuleSummary(state.summary ?? '', { complete: state.lessonCompleted })}<button class="spec-action-button" type="button" data-spec-action="complete-lesson" ${state.lessonCompleted ? 'disabled' : ''}>${state.lessonCompleted ? 'Lessonを完了しました' : 'Lessonを完了する →'}</button></div>`;
...
export function mountParticipleLesson(root, lesson, { onProgress } = {}) {
  ...
  const restoredFinalOutcome = restoredFinalAttempt
    ? (getFinalScores(finalStage, savedLesson.quizResults ?? {}).answerScore >= 4 ? 'complete' : 'review')
    : null;
  const state = {
  ...
  on(root, 'click', (event) => {
    ...
    if (action) {
      ...
      if (actionName === 'intro-select') state.introSelected = value;
        ...
        progress[lessonKey].answerScore = scores.answerScore;
        progress[lessonKey].reasoningScore = scores.reasoningScore;
        if (scores.answerScore >= 4) { state.finalOutcome = 'complete'; markStageComplete(); } else { state.finalOutcome = 'review'; persistState(); }
      } else if (actionName === 'final-retry') { state.questions = {}; state.finalOutcome = null; persistState(); }
    ...
    if (caseRelation) { const [indexText, relation] = caseRelation.dataset.specCaseRelation.split(':'); const index = Number(indexText); const answer = currentStage().cases?.[index]?.relationAnswer; setCaseState(index, relation === answer ? { relation, error: false } : { error: true }); render(); return; }
    const caseForm = event.target.closest('[data-spec-case-form]');
    if (caseForm) { const [indexText, form] = caseForm.dataset.specCaseForm.split(':'); const index = Number(indexText); const answer = currentStage().cases?.[index]?.formAnswer; setCaseState(index, form === answer ? { form, error: false } : { error: true }); render(); return; }
    ...
    if (finalForm) {
      ...
      state.questions[questionId] = { ...previous, form, formAttempts: (previous.formAttempts ?? 0) + 1, formCorrect, correct: previous.relationCorrect === true && formCorrect };
      persistState();
      render();
      return;
    }
